...
import { createNewSponsorUser } from '../util/users';
...
const AdminAddNewSponsorUserPage = () => {
  ...
  const { mutate, isPending, isSaveError, saveError } = useMutation({
    mutationFn: createNewSponsorUser,
    onSuccess: () => {
  ...
  return (
    <>
      {showErrorAlert && (
        <CustomAlert
          type='error'
          message='Sponsor user created failed!'
      ...
      {showSuccessAlert && (
        <CustomAlert
          type='success'
          message='Sponsor user created sucessfully!'
...
export default AdminAddNewSponsorUserPage;
